...
def launch_eval(args):
    ...
    if args.run_name is not None:
        run_name = args.run_name
    # Else option 2: Check if a config has been                                                                      to look for the run_name
    if args.config is not None:
        config = run_utils.load_config(args.config)
        if config is not None and "run_name" in config and run_name is None:
            run_name = config["run_name"]
    ...
    torch.multiprocessing.spawn(eval_process, nprocs = args.gpus , args=(config, shared_dict, barrier)) # tried nprocs = 1 to solve SIGKILL
